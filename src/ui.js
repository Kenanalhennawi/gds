...
export const renderTimeline = (container, { events, summary }) => {
    ...
    // 1. Render Summary Dashboard (if data exists)
    if (summary && events.length > 0) {
        ...
        summaryCard.style.borderLeft = `4px solid var(--neon-${summary.alertLevel === 'critical' ? 'red' : summary.alertLevel === 'warning' ? 'gold' : 'green'})`;
